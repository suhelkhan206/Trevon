...
import com.tws.trevon.activity.sp.MyBankDetails2;
...
public class MyBankDetails extends AbstractActivity {
    ...
    public boolean chekAddrValidation()
    {
        ...
        if (acc_no.getText().toString().trim().length() == 0) {
            isValid = false;
            acc_no.requestFocus();
            Toast.makeText(MyBankDetails.this, "Enter Account Number", Toast.LENGTH_SHORT).show();
        }
        else if (cacc_no.getText().toString().trim().length() == 0) {
            isValid = false;
            cacc_no.requestFocus();
            Toast.makeText(MyBankDetails.this, "Enter Confirm Account Number", Toast.LENGTH_SHORT).show();
        }
        else if(!acc_no.getText().toString().trim().equals(cacc_no.getText().toString().trim()))
        {
            isValid = false;
            cacc_no.requestFocus();
            Toast.makeText(MyBankDetails.this, "Account and Confirm Account Number not match", Toast.LENGTH_SHORT).show();
        }
        else if (acc_hname.getText().toString().trim().length() == 0) {

            isValid = false;
            acc_hname.requestFocus();
            Toast.makeText(MyBankDetails.this, "Enter Account holder name", Toast.LENGTH_SHORT).show();
        }
        else if (bank_name.getText().toString().trim().length() == 0) {

            isValid = false;
            bank_name.requestFocus();
            Toast.makeText(MyBankDetails.this, "Enter bank name", Toast.LENGTH_SHORT).show();
        }
        else if (ifsc.getText().toString().trim().length() == 0) {

            isValid = false;
            ifsc.requestFocus();
            Toast.makeText(MyBankDetails.this, "Enter IFSC code", Toast.LENGTH_SHORT).show();
        }
        ...
        if(phone_pay_number.getText().toString().trim().length() > 0)
        {
            if(!AppValidate.isMobileNumber(phone_pay_number.getText().toString()))
            {
                ...
                Toast.makeText(MyBankDetails.this, "Enter Correct Phonepay Number", Toast.LENGTH_SHORT).show();
            }
        }
         if(google_pay_number.getText().toString().trim().length() > 0)
        {
            if(!AppValidate.isMobileNumber(google_pay_number.getText().toString()))
            {
                ...
                Toast.makeText(MyBankDetails.this, "Enter Correct GooglePay Number", Toast.LENGTH_SHORT).show();
            }
        }
         if(paytm_number.getText().toString().trim().length() > 0)
        {
            if(!AppValidate.isMobileNumber(paytm_number.getText().toString()))
            {
                ...
                Toast.makeText(MyBankDetails.this, "Enter Correct Paytm Number", Toast.LENGTH_SHORT).show();
            }
        }
         if(amazone_pay_number.getText().toString().trim().length() > 0)
        {
            if(!AppValidate.isMobileNumber(amazone_pay_number.getText().toString()))
            {
                ...
                Toast.makeText(MyBankDetails.this, "Enter Correct AmazonPay Number", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private void selectImage()
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(MyBankDetails.this);
    ...
    private boolean hasCameraPermission() {
        return ContextCompat.checkSelfPermission(MyBankDetails.this,
    ...
    private void captureFromCamera() {
        try {
            Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            intent.putExtra(MediaStore.EXTRA_OUTPUT, FileProvider.getUriForFile(MyBankDetails.this, BuildConfig.APPLICATION_ID + ".provider", createImageFile()));
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK)
        {
            if (requestCode == 1)
            {
                ...
                try {
                    File orignalFile = FileUtils.from(MyBankDetails.this, selectedImage);
    ...
    {
        pDialog = new ProgressDialog(MyBankDetails.this);
        ...
        call.enqueue(new okhttp3.Callback() {
            ...
            public void onFailure(okhttp3.Call call, IOException e) {
                try {
                    pDialog.dismiss();
                    Toast.makeText(MyBankDetails.this, "Network error.", Toast.LENGTH_SHORT).show();
                } catch (Exception ex) {
                    e.addSuppressed(ex);
                }
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        {
            Intent i= new Intent(MyBankDetails.this, SearchPage.class);
            startActivity(i);
        }
        ...
        else if (item.getItemId()==R.id.notification_home)
        {
            ...
            {
                Intent i =new Intent(MyBankDetails.this,Notification.class);
                startActivityForResult(i, IConstants.INTENT_CART_COUNT_CODE);// Activity is started with requestCode 2
            }
            else
            {
                Toast.makeText(this, "Login First", Toast.LENGTH_SHORT).show();
                Intent ilogin = new Intent(MyBankDetails.this,LoginOriginal.class);
        ...
        else if (item.getItemId()==R.id.action_cart)
        {
            ...
            if(AppValidate.isNotEmpty(userCO.id))
            {
                Intent i =new Intent(MyBankDetails.this,CartScreen.class);
                startActivityForResult(i, IConstants.INTENT_CART_COUNT_CODE);// Activity is started with requestCode 2
            }
            else
            {
